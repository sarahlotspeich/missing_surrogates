...
## Gold standard + complete case + IPW 
p = paste0("https://raw.githubusercontent.com/sarahlotspeich/missing_surrogates/refs/heads/main/simulations/sett4_misspec_ipw_overfit/sett4_misspec_ipw_overfit_seed", 0:19, ".csv")
sim_res = do.call(dplyr::bind_rows, 
                  lapply(X = paste0(p, list.files(p)), 
                         FUN = read.csv))
# SMLE (from Setting 2)
...
## Save combined data 
sim_res |> 
  write.csv("simulations/sett4_misspec_ipw_overfit.csv", 
            row.names = FALSE)
